...
--// Compatibility (CORRIGIDO PARA SOLARA) //--
local request = (syn and syn.request) or http_request or request or (http and http.request) or fluxus and fluxus.request
local getcustomasset = getsynasset or getcustomasset
local isfolder = isfolder or isfile -- Solara usa isfile
local makefolder = makefolder or writefile and function(path) 
    -- Fallback para Solara
    pcall(function() writefile(path .. "/.init", "") end)
end
...
-- SISTEMA DE DOWNLOAD DE ASSETS (CORRIGIDO)
local function DownloadAssets()
    local success, err = pcall(function()
        if not isfolder or not isfolder("PPHUD") then
            if makefolder then
                makefolder("PPHUD")
            end
            ...
            if request and writefile then
                -- Download Arrow
                local Arrow = request({
                    Url = "https://raw.githubusercontent.com/Rain-Design/PPHUD/main/Dropdown.png", 
                    Method = "GET"
                })
                if Arrow and Arrow.Body then
                    writefile("PPHUD/Arrow.png", Arrow.Body)
                end
                
                -- Download Resize
                local Resize = request({
                    Url = "https://raw.githubusercontent.com/Rain-Design/PPHUD/main/resize.png", 
                    Method = "GET"
                })
                if Resize and Resize.Body then
    ...
    if not success then
        warn("[PPHUD] Erro ao baixar assets:", err)
    end
end

-- Tenta baixar os assets
task.spawn(DownloadAssets)
...
-- Função para obter asset de forma segura
local function SafeGetAsset(path)
    if getcustomasset and pcall(function() return readfile(path) end) then
...
function library:Window(WindowArgs)
        ...
        Utilities:Create("Frame", {
            ...
            Utilities:Create("Frame", {
                ...
                Utilities:Create("ImageLabel", {
                    ...
                    BackgroundTransparency = 1,
                    Image = SafeGetAsset("PPHUD/Resize.png"),
    ...
    -- PROTEÇÃO DA GUI (CORRIGIDO PARA SOLARA)
                    ...
                    Utilities:Create("Frame", {
                        ...
                        Utilities:Create("Frame", {
                            ...
                            Utilities:Create("ImageLabel", {
                                ...
                                BackgroundTransparency = 1,
                                Rotation = 0,
                                Image = SafeGetAsset("PPHUD/Arrow.png"),
                                AnchorPoint = Vector2.new(.5, .5),
                                Position = UDim2.new(.5, 0, .5, 0),
                                ZIndex = DropdownTable.Index
                            })
